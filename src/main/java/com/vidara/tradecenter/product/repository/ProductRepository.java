...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    @Query("SELECT p FROM Product p WHERE " +
            "LOWER(CAST(p.name AS string)) LIKE LOWER(:search) OR " +
            "LOWER(CAST(p.description AS string)) LIKE LOWER(:search)")
    ...
    @Query("SELECT p FROM Product p WHERE " +
            ...
            "(:search IS NULL OR LOWER(CAST(p.name AS string)) LIKE LOWER(:search) OR " +
            "LOWER(CAST(p.description AS string)) LIKE LOWER(:search))")
